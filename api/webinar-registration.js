...
module.exports = async function handler(req, res) {
  ...
  emailData.append('Email address', registration.emailAddress);
  emailData.append('Current status', registration.currentStatus);
  emailData.append('Total experience', registration.totalExperience);
  emailData.append('Selected interests', registration.interests.join(', ') || 'Not specified');
  ...
  emailData.append('Source page URL', registration.sourcePageUrl || 'https://www.testnova.in/registration/');

  try {
    const notification = await fetch('https://formsubmit.co/ajax/admin@testnova.in', {
      method: 'POST', headers: { Accept: 'application/json' }, body: emailData
    });

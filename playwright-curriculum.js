(function () {
  ...
  window.TestNovaPlaywrightCurriculum = [
    ...
    topic(58, 'AI-Powered E2E Testing with Playwright MCP and GitHub MCP', [
      'Model Context Protocol (MCP) lets an AI coding assistant use approved tools instead of relying only on pasted text. Playwright MCP gives the assistant structured browser access for exploring and testing a web application. GitHub MCP supplies repository, issue, pull-request, and workflow context according to the permissions you grant.',
      'The AI assistant accelerates discovery, test drafting, and failure analysis, but the engineer remains responsible for the test intent, permissions, generated changes, and final review. Treat page content, issue text, and repository files as untrusted input: never expose secrets, production customer data, or unrestricted write credentials.',
      'MCP configuration differs between hosts. Follow the documentation for your editor or AI client, approve only the tools needed for the task, and start GitHub access in read-only mode when write operations are unnecessary.'
    ], [
      example('Step 1 - connect Playwright MCP', '{\n  "servers": {\n    "playwright": {\n      "command": "npx",\n      "args": ["@playwright/mcp@latest"]\n    }\n  }\n}', 'Use the configuration format required by your MCP host. Pin an approved version when reproducible team environments require it.', 'json'),
      example('Step 2 - connect GitHub with least privilege', 'Authenticate the official GitHub MCP server with OAuth or a fine-grained token.\nGrant access only to the required repository and operations.\nUse read-only mode when no writes are needed.', 'Never paste a token into chat, source code, or a committed configuration file.', 'text'),
      example('Step 3 - turn the issue into a test contract', 'Read issue #123 and summarize:\n- the user journey\n- preconditions and test data\n- expected outcomes\n- edge cases\n- ambiguous acceptance criteria\nDo not change repository files yet.', 'GitHub MCP can retrieve the issue and related repository context. Resolve unclear criteria before generating a test.', 'text'),
      example('Step 4 - explore the UI with Playwright MCP', 'Open the local test environment.\nComplete the checkout journey from issue #123.\nInspect accessible page state after each major step.\nRecord stable roles, labels, test IDs, URLs, and visible outcomes.\nDo not submit a real payment or modify production data.', 'Browser exploration grounds the test in the current interface. Prefer semantic locators discovered from accessible page state.', 'text'),
      example('Step 5 - request a focused implementation', 'Create tests/checkout.spec.ts for the confirmed happy path.\nUse getByRole(), getByLabel(), or approved test IDs.\nUse web-first expect assertions for user-visible outcomes.\nReuse existing fixtures and project conventions.\nDo not add fixed waits, real credentials, or unrelated refactors.', 'Review the proposed diff before accepting it. Generated code must follow the repository conventions and security rules.', 'text'),
      example('Step 6 - run and diagnose locally', 'npx playwright test tests/checkout.spec.ts\nnpx playwright test tests/checkout.spec.ts --debug\nnpx playwright show-report', 'Run the focused test first. If it fails, inspect the error, trace, screenshot, and live page state before changing the assertion.', 'bash'),
      example('Step 7 - request an evidence-based repair', 'Explain the failure using the Playwright error and trace.\nClassify it as a product defect, environment issue, test-data issue, or test-code issue.\nPropose the smallest fix and show the diff before editing.', 'Do not weaken an assertion merely to make the test pass. Expected behavior must match the acceptance criteria.', 'text'),
      example('Step 8 - collaborate through GitHub MCP', 'Create a branch and pull request only after local validation.\nLink issue #123.\nSummarize the scenario covered and commands run.\nInclude relevant evidence, but exclude secrets and personal data.\nCheck CI and report failures; do not merge without approval.', 'Enable GitHub write tools explicitly. Keep a human approval step for publishing and merging changes.', 'text'),
      example('Example generated Playwright test', "import { test, expect } from '@playwright/test';\n\ntest('signed-in shopper completes checkout', async ({ page }) => {\n  await page.goto('/cart');\n  await page.getByRole('button', { name: 'Checkout' }).click();\n  await page.getByLabel('Email').fill('shopper@test.example');\n  await page.getByRole('button', { name: 'Place order' }).click();\n\n  await expect(page.getByRole('heading', {\n    name: 'Order confirmed'\n  })).toBeVisible();\n});", 'Replace the example route, data, and accessible names with values confirmed in the target application.')
    ], [
      flow('AI-Assisted E2E Workflow', ['Connect least-privilege MCP servers', 'Read the issue and repository conventions', 'Explore the UI with Playwright MCP', 'Confirm the test contract', 'Generate and review a focused test', 'Run locally and inspect evidence', 'Publish a reviewed pull request', 'Monitor CI and obtain approval']),
      checklist('Human Review Gate', ['Acceptance criteria are clear', 'Target is a safe test environment', 'Locators are stable and user-focused', 'Assertions verify meaningful outcomes', 'No fixed waits or hidden flaky retries', 'No secrets or personal data in code or artifacts', 'Diff contains only intended changes', 'Focused test and relevant suite pass', 'CI result is reviewed before merge'])
    ])
  ];
})();

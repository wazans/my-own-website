(function () {
  ...
  function buildPlaywrightJsTsTopics() {
    var items = [
      ...
      ['Hoisting', 'Level 1 - JavaScript Foundations', 'Scope', 'Hoisting explains why some declarations are available before the line where they appear. Function declarations are hoisted, while let and const remain unavailable before declaration.', 'For clean automation code, declare variables before using them and avoid relying on hoisting tricks.', [{ title: 'Hoisting basics', code: 'sayHello();\n\nfunction sayHello() {\n  console.log("Hello");\n}\n\n// console.log(userName); // error with let/const before declaration\nconst userName = "qa_user";' }]],
      ...
      ['Arrays', 'Level 2 - Working with Data', 'Loops and Data Types', 'Arrays store multiple values in one variable, such as users, roles, products, countries, or expected table rows. Access values by index and loop through them.', 'Arrays are the base for data-driven Playwright tests.', [{ title: 'Array test data', code: 'const tools = ["java", "playwright", "git", "github"];\nconsole.log(tools[2]); // git\nconsole.log(tools[5]); // undefined\n\nconst userData = ["Wasim", 32, true, null, "Noida"];\nconsole.log(userData[4]);' }]],
